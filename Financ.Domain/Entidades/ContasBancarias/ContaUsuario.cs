...
namespace Financ.Domain.Entidades.ContasBancarias
{
    public sealed class ContaUsuario : EntidadeBase
    {
        ...
        public ExpiracaoBase? Expiracao { get; private set; }
        public PreferenciaContaUsuario Preferencias { get; } = PreferenciaContaUsuario.Create();

        private ContaUsuario(Conta conta, Usuario usuario,ETiposAcessos acessos)
        {
            ValidaNullo.Verifica(conta, MensagensBase.CONTA_NULA);
            ValidaNullo.Verifica(usuario, MensagensBase.USUARIO_NULO);
            Acesso = acessos;
            Conta = conta;
            Usuario = usuario;
        }
        ...
        public static ContaUsuario CriarPrimeiroUsuario(Conta conta, Usuario usuario)
        {
        ...
        public void AtualizaOutraContaUsuario(ContaUsuario? contasUsuarioRemetente, ETiposAcessos? acesso, EStatusContasUsuario? status, int? expiracao = null, bool? removerExpiracao = null)
        {
            ...
            {
                ContasUsuariosValidacao.Verifica(ValidaExpiracao(expiracao.Value), MensagensContaUsuario.TEMPO_MIN_EXPIRACAO);
                Expiracao = DateTime.UtcNow.AddMinutes(expiracao.Value);
            }

            if (removerExpiracao.HasValue)
            {
                ContasUsuariosValidacao.Verifica(expiracao.HasValue && (removerExpiracao.Value), MensagensContaUsuario.CONFLITO_AO_EXPIRAR);
        ...
        private void ValidaContaBancaria(Conta conta)
        {
