...
namespace Financ.Infra.Data.Repositorios
{
    public class ConvitesRepositorio : BaseRepositorio<Convites>, IConvitesRepostorio
    {
        ...
        public async Task<Convites> BuscarConviteComContasEContasUsuarios(Expression<Func<Convites, bool>> predicado)
        {
            return await _contexto.Convites.Include(c => c.Conta)
                .ThenInclude(c => c.ContaUsuarios)
                .Where(x => x.Aceito == null && x.Expiracao >= DateTime.UtcNow).FirstOrDefaultAsync(predicado);
        }

        public async Task<IEnumerable<Convites>> ObterConviteComRemetenteDestinatarioEContaAsync(Expression<Func<Convites, bool>> predicado)
        {
            return await _contexto.Convites.Include(c => c.Conta)
